...
export default function AccountCenter({user,api,onUser,onPassword}:{user:any,api:Api,onUser:(user:any)=>void,onPassword:()=>void}){
 ...
 <form onSubmit={e=>{e.preventDefault();const form=e.currentTarget,d=new FormData(form);save(async()=>{const changed=username.trim().toLowerCase()!==user.username;const r=await api('account/profile',{name:d.get('name'),username:d.get('username'),current:d.get('current')});onUser(r.user);setUsername(r.user.username);const current=form.elements.namedItem('current') as HTMLInputElement|null;if(current)current.value='';setNotice(changed?'Profile saved. Use your new username next time you sign in. Other sessions have been signed out.':'Profile saved.')})}}><label>Username<Input name="username" value={username} onChange={e=>setUsername(e.target.value)} required pattern="[a-zA-Z0-9_.-]{3,40}" maxLength={40} autoCapitalize="none" autoComplete="username" spellCheck={false}/><small>3–40 letters, numbers, periods, hyphens or underscores.</small></label><label>Display name<Input name="name" defaultValue={user.name} required maxLength={80} autoComplete="name"/></label>{username.trim().toLowerCase()!==user.username&&<label>Current password<Input name="current" type="password" required maxLength={128} autoComplete="current-password"/><small>Confirm your password to change your username.</small></label>}<Button disabled={busy}>Save profile</Button></form></section>
