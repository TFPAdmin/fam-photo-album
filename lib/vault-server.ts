...
async function route(r:Request):Promise<Response>{
 ...
 if(p[0]==='password'&&method==='POST'){const b=await body(r);await throttle('password:'+u.id,12);if(!eq(await pw(String(b.current||''),u.password.split(':')[0]),u.password))fail('Current password is incorrect.');await db().batch([stmt('UPDATE users SET password=?,must_change=0 WHERE id=?',await pw(password(b.password)),u.id),stmt('DELETE FROM sessions WHERE user_id=? AND token<>?',u.id,u.session_token)]);return json({user:profile({...u,must_change:0})})}
