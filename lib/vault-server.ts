...
async function user(r:Request){const token=(r.headers.get('cookie')||'').match(/(?:^|; )auth=([^;]+)/)?.[1];if(!token)fail('Please sign in.',401);const u=await one('SELECT u.* FROM users u JOIN sessions s ON s.user_id=u.id WHERE s.token=? AND s.expires>? AND u.active=1',await sha(encode(token)),now());if(!u)fail('Please sign in again.',401);return u}
...
async function route(r:Request):Promise<Response>{
 ...
 if(p[0]==='logout'&&method==='POST'){const t=(r.headers.get('cookie')||'').match(/(?:^|; )auth=([^;]+)/)?.[1]||'';await stmt('DELETE FROM sessions WHERE token=?',await sha(encode(t))).run();return json({ok:true},200,{'Set-Cookie':'auth=; Path=/; Secure; HttpOnly; SameSite=Strict; Max-Age=0'})}
 if(p[0]==='password'&&method==='POST'){const b=await body(r);await throttle('password:'+u.id,12);if(!eq(await pw(String(b.current||''),u.password.split(':')[0]),u.password))fail('Current password is incorrect.');await db().batch([stmt('UPDATE users SET password=?,must_change=0 WHERE id=?',await pw(password(b.password)),u.id),stmt('DELETE FROM sessions WHERE user_id=?',u.id)]);return session({...u,must_change:0})}
 if(u.must_change)fail('Change your temporary password before continuing.',403);
 if(p[0]==='members'){
  if(method==='GET')return json({members:await all('SELECT id,username,name,role,active FROM users WHERE active=1 OR ?=1 ORDER BY name',u.role==='owner'||u.role==='admin'?1:0)});
