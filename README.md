...
Recently removed files remain recoverable and continue to use storage. Individual permanent-file deletion is not exposed; the primary admin can permanently delete an account and all of its uploads as described below. Browser support varies for HEIC/HEIF and video codecs; download originals into a compatible application where necessary. To preserve Live Photos, export/upload both photo and video components. A phone's file picker can apply its own export conversion before the application receives the file.
...
## Primary admin: full account management

In **Family accounts → Manage account**, the primary admin can edit any Member or Admin account’s name, username, access level, enabled state and requirement to change its password. A new password is optional; it must be confirmed. Recovery questions can be kept, cleared, or replaced with three new questions/answers. Existing passwords and answers are never displayed. A password replacement clears old recovery questions unless replacement answers are provided in the same save. Saving revokes that account’s sessions and clears its login/recovery attempt counters. Profile changes preserve account IDs, media ownership, albums and sharing. Regular admins do not receive these full editing/deletion controls.

**Delete account permanently** requires typing the account’s current username exactly. Deletion disables the account, revokes sessions and sharing, aborts tracked multipart uploads, deletes originals/thumbnails (including recently removed media), and removes media records, parts, albums, incoming shares, credentials, recovery settings and account-specific attempt counters. Files uploaded by other accounts are not deleted. Cleanup is limited to the selected account’s UUID storage prefix. The primary admin cannot delete or reassign their own protected account.
